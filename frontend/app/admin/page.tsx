...
export default function AdminPage() {
  ...
  if (!user || !token || !isAdmin) {
    return (
      <main className="max-w-md mx-auto px-5 py-24 text-center">
        ...
        <p className="text-char/60">העמוד הזה זמין רק למנהלי האתר.</p>
      </main>
    );
  }
  ...
  return (
    <main className="max-w-2xl mx-auto px-5 py-12">
      ...
      {messages.length === 0 ? (
        <p className="text-char/60">אין עדיין פניות.</p>
      ) : (
        <div className="flex flex-col gap-4">
          {messages.map((m) => (
            <div key={m.id} className="border border-char/20 p-4">
              <div className="flex items-center justify-between mb-2 text-sm">
                <span className="font-bold">{m.name}</span>
                <span className="text-char/50 text-xs">
                  {new Date(m.created_at).toLocaleString("he-IL")}
                </span>
              </div>
              <a href={`mailto:${m.email}`} className="text-oxide text-xs hover:underline">
                {m.email}
              </a>
              <p className="text-char/80 text-sm mt-2 whitespace-pre-line">{m.message}</p>
            </div>
          ))}
        </div>
      )}
